...
namespace StayInTarkov.Coop.Components.CoopGameComponents
{
    public class SITGameGUIComponent : MonoBehaviour
    {
        ...
        private Rect DrawPing(Rect rect)
        {
            ...
            Color textColor;
            if (gameclient.Ping > 150)
            {
                textColor = Color.red;
            }
            else if (gameclient.Ping > 70)
            {
            ...
            var text = new GUIContent($"{(SITMatchmaking.IsClient ? "客户端" : "主机")} 延迟:{gameclient.Ping} 上传:{gameclient.UploadSpeedKbps:0.00}KB  下载:{gameclient.DownloadSpeedKbps:0.00}KB  丢包率:{gameclient.PacketLoss:0}% {(AkiBackendCommunication.Instance.HighPingMode ? "hpm" : "")}");
